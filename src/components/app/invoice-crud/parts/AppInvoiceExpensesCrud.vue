  ...
  <div class="space-y-8">
    <div class="flex gap-3">
      <base-label-input
        ...
        id="invoice_item"
        label="Mal Hizmet"
        v-model="modalForm.invoiceItem"
      />
      <base-label-input
        class="w-full"
        id="quantity"
        label="Miktar"
        v-model="modalForm.quantity"
      />
    </div>
    ...
    <div class="flex gap-3">
      <base-label-input
        ...
        id="quantity_price"
        label="Birim Fiyat"
        v-model="modalForm.quantityPrice"
      />
      <base-label-input
        class="w-full"
        id="tax_rate"
        label="KDV Oranı"
        v-model="modalForm.tax_rate"
      />
    </div>
    ...
    >
      <div class="text-end">
        <base-icon-button
          icon-class="icofont-ui-delete"
          text="Satırı Sil"
          @click="deleteLine(key, 'rows')"
    ...
    <div class="text-end">
      <base-icon-button
        @click="addLine('countRow', 'rows')"
        class="btn-success"
        text="Yeni Satır Ekle"
        icon-class="icofont-plus text-sm"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...mapActions(useInvoiceStore, ["deleteLine", "addLine"]),
  },
};
</script>
